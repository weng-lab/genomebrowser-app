...
export default function DomainButtons({ browserStore }: { browserStore: BrowserStoreInstance }) {
      ...
      <Box display={"flex"} flexDirection={"row"} alignItems={"center"}>
        {buttons.map((btn, index) => {
          return (
            ...
            >
              <Typography variant="body2" display="flex" alignItems="center" justifyContent="center" pt={0.5}>
                {btn.label}
              </Typography>
            </Button>
          );
    ...
    <Box justifyContent={"space-around"} flexDirection={"row"} display={"flex"} width={"100%"}>
      {buttonGroups.map((group, index) => (
        <Box key={index}>
          ...
          {index !== buttonGroups.length - 1 && <Divider orientation="vertical" flexItem />}
        </Box>
      ))}
    </Box>
  );
